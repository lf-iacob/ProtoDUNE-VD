...
particle_by_type['HxL'] = 'kCTBOffSpillSnapshot'
...
triggertypedaq['total'] = np.sum( [ value for key, value in triggertypedaq.items() if key not in ['kCTBBeamChkvHxL', 'kCTBBeamSpillStart', 'total'] ] ) 
triggertypedaq
...
def filter_offspill(waveform: Waveform, specifictype="") -> bool:
    if 'kCTBBeamChkvHxLx' in waveform.trigger_type_names or 'kCTBBeamSpillStart' in waveform.trigger_type_names:
        return False
    return True
