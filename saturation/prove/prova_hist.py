...
The main features for each run done during period 20-28 august are shown in the table: energy, target,
level of high and low preassure for Cherenkov trigger, type of particle depending on the region indica-
ted by the Cherenkov thresholds. For each module, the number of photolectrons detetcted by event by the
pair of channels is calculated as the ratio between the energy collected (charge: integral of the wfset)
and the single photoelectron charge. The collected data is specific for each type of configuration of 
the Cherenkov triggers, so to being able to recognise the impact on the detectors operated by the dif-
ferent types of particles.
P.S. From the signal only not saturated wfs are selected. Still, a small part of the saturated wfs cau-
sed by the cold electronics are still taken to calculate the charge, but these are not abundant (only
present in the membrane) and their impact not relevant.

The aim of this notebook is taking the infos registered in the spreadsheet to create histograms, repre-
senting #peVSmodules. Would be nice to have a 3D visualisation of the histograms as a lego plot, taking
into account the real position of the modules in the real ProtoDUNE-VD detector.

Other plot represents the total number of pe detected by the PDS VS the energy of the single run. It 
represents the energy loss in terms of total photoelectrons emetted in the specific run. It should be
linear, since the more energy the particles have, the more photoelectrons should be produced.
...
import numpy as np
import pandas as pd
    #%matplotlib notebook (for interactive 3D plots on Jupyter notebook)
    #%matplotlib inline (to go back to python)
...
#exctract the name of the ordered modules form the DataFrame
modules = df_original.columns[7:-1].tolist()

#DataFrame without bad modules (M1, M2, C3, C5)
df = df_original.copy()
df.iloc[:, [9, 10, 14, 15]] = 0
...
axs[0, 0].set_ylabel('#pe', fontweight='bold')
    # - HL
...
axs[0, 1].set_ylabel('#pe', fontweight='bold')
    # - HLx
...
axs[1, 0].set_ylabel('#pe', fontweight='bold')
    # - HxLx
...
axs[1, 1].set_ylabel('#pe', fontweight='bold')
...
ax.set_zlabel('#pe', fontweight='bold')
...
bx.set_zlabel('#pe', fontweight='bold')
...
cx.set_zlabel('#pe', fontweight='bold')
...
ex.set_zlabel('#pe', fontweight='bold')
...
'''
MIO TENTATIVO - CORRETTO DA HENRIQUE SOTTO (per separare energia negativa e positiva con marker diverso)
#peVSE
...
plt.scatter(energy, pe_sum_total, color='mediumseagreen', label='Total')
plt.scatter(energy, pe_sum_HL, color='mediumturquoise', label='HL')
plt.scatter(energy, pe_sum_HLx, color='lightcoral', label='HLx')
plt.scatter(energy, pe_sum_HxLx, color='darkviolet', label='HxLx')
plt.legend()
plt.xlabel('Energy [GeV]', fontweight='bold')
xticks = np.arange(np.floor(min(energy)*2)/2, np.ceil(max(energy)*2)/2 + 0.5, 0.5)
plt.xticks(xticks)
plt.ylabel('Sum #pe', fontweight='bold')
plt.show()
'''
