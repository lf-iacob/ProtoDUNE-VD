...
class TimeResolution:
    ...
    def create_wfs(self) -> None:
        '''
        Metodo che prende il wfset in analisi per questa time resolution (self.wf_set) e filtra solo i canali permessi.
        Dopo prende le wfs valutate buone dal filtraggio e valuta delle caratteristiche.
        '''
        #le info che non sai da dove vengono sono in utils.py che stanno nella stessa cartella time_resolution
        t_wfset = waffles.WaveformSet.from_filtered_WaveformSet(self.wf_set, allow_channel_wfs, self.ch)
            #allow_channel_wfs: controlla che il canale scritto è corretto: endpoint e channel number hanno senso
        self.wfs = t_wfset.waveforms #prende le wfs dal set (metodo della classe WaveformSet) appena ottenuto dall'operazione di filtraggio
        create_float_waveforms(self.wfs) #per ciascuna wf traduce i canali dell'adc in tipo float64
        sub_baseline_to_wfs(self.wfs, self.prepulse_ticks) #calcola la baseline e la sottrae al segnale - riscala tutti i canali adc
