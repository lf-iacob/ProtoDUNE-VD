...
def create_float_waveforms(waveforms: waffles.Waveform) -> None:
    #per ciascuna wfs prende i canali dell'adc e li converte in float64
    for wf in waveforms:
        wf.adcs_float = wf.adcs.astype(np.float64)

def sub_baseline_to_wfs(waveforms: waffles.Waveform, prepulse_ticks: int):
    norm = 1./prepulse_ticks #calcolo per ottenere la normalizzazione (computa la media)
    for wf in waveforms:
        baseline = np.sum(wf.adcs_float[:prepulse_ticks])*norm #calcola la baseline
        wf.adcs_float -= baseline #toglie il valore della baseline ad ogni canale adc per avere il segnale sullo 0
        wf.adcs_float *= -1 #inverte (tesi di Galizzi: il segnale capovolto rispetto ad asse x)
